...
import net.sourceforge.c4jplugin.internal.ui.contracthierarchy.actions.FocusOnSelectionAction;
import net.sourceforge.c4jplugin.internal.ui.contracthierarchy.actions.FocusOnTypeAction;
import net.sourceforge.c4jplugin.internal.ui.contracthierarchy.actions.HistoryDropDownAction;
...
public class ContractHierarchyViewPart extends ViewPart {
	...
	private ContractHierarchyLifeCycle fHierarchyLifeCycle;
	...
	public ContractHierarchyViewPart() {
		fSelectedType= null;
		fInputElement= null;
		fIsVisible= false;
		...
		fHierarchyLifeCycle= new ContractHierarchyLifeCycle();
		fTypeHierarchyLifeCycleListener= new IContractHierarchyLifeCycleListener() {
			public void contractHierarchyChanged(ContractHierarchyLifeCycle typeHierarchy, IType[] changedTypes) {
		...
		fPartListener= new IPartListener2() {
			public void partVisible(IWorkbenchPartReference ref) {
				...
				if (part == ContractHierarchyViewPart.this) {
	...
	public void setInputElement(IJavaElement element) {
		...
		if (element != null) {
			if (element instanceof IMember) {
				...
				if (!element.exists()) {
					MessageDialog.openError(getSite().getShell(), ContractHierarchyMessages.TypeHierarchyViewPart_error_title, ContractHierarchyMessages.TypeHierarchyViewPart_error_message); 
					return;
				}
			} else {
	...
	public void dispose() {
		...
		if (fPropertyChangeListener != null) {
			C4JActivator.getDefault().getPreferenceStore().removePropertyChangeListener(fPropertyChangeListener);
	...
	private Control createTypeViewerControl(Composite parent) {
		...
		initializeTypesViewer(superTypesViewer, keyListener, "net.sourceforge.c4jplugin.ui.contracthierarchy.tree.supercontracts");
		
		StructuredViewer subTypesViewer= new SubContractHierarchyViewer(fViewerbook, fHierarchyLifeCycle, this);
		initializeTypesViewer(subTypesViewer, keyListener, "net.sourceforge.c4jplugin.ui.contracthierarchy.tree.subcontracts");
	...
	private void initializeTypesViewer(final StructuredViewer typesViewer, KeyListener keyListener, String cotextHelpId) {
		...
		if (typesViewer instanceof AbstractStructuredGraphViewer) {
	...
	public void createPartControl(Composite container) {
		...
		IMenuManager layoutSubMenu= new MenuManager(ContractHierarchyMessages.TypeHierarchyViewPart_layout_submenu); 
	...
	private void updateHierarchyViewer(final boolean doExpand) {
		if (fInputElement == null) {
			...
			if (((IContractHierarchyViewer)getCurrentViewer()).containsElements() != null) {
				...
				fEmptyTypesViewer.setText(NLS.bind(ContractHierarchyMessages.TypeHierarchyViewPart_nodecl, fInputElement.getElementName()));				 
				setViewerVisibility(false);
			}
		}
	}
	...
	private void updateTitle() {
		...
		if (fInputElement != null) {
			...
			if (workingSet == null) {
				...
				title= NLS.bind(ContractHierarchyMessages.TypeHierarchyViewPart_title, args); 
				tooltip= NLS.bind(ContractHierarchyMessages.TypeHierarchyViewPart_tooltip, args); 
			} else {
				...
				title= NLS.bind(ContractHierarchyMessages.TypeHierarchyViewPart_ws_title, args); 
				tooltip= NLS.bind(ContractHierarchyMessages.TypeHierarchyViewPart_ws_tooltip, args); 
			}
		} else {
			title= ""; //$NON-NLS-1$
			tooltip= viewerTitle;
	 ...
	 */
	protected void doTypeHierarchyChanged(final ContractHierarchyLifeCycle typeHierarchy, final IType[] changedTypes) {
		if (!fIsVisible) {
			fNeedRefresh= true;
			return;
		}
	...
	private void restoreState(final IMemento memento, IJavaElement defaultInput) {
		...
		if (input == null) {
			...
			synchronized (this) {
				String label= NLS.bind(ContractHierarchyMessages.TypeHierarchyViewPart_restoreinput, hierarchyInput.getElementName()); 
